...
A checkbox component with square and circle shapes, optional label text, and enabled/disabled states. Supports interactive toggle and brand-aware theming.

**Shapes:**

| Shape | Description |
|-------|-------------|
| `SQUARE` | Rounded rectangle (6dp corners), stroke border when unchecked, filled when checked |
| `CIRCLE` | Oval shape, stroke border when unchecked, filled when checked |
...
- Layout inflated from `res/layout/view_checkbox.xml` using `<merge>` tag
- Indicator drawn programmatically via `CheckboxDrawable` (custom `Drawable`)
- Checkmark path extracted from `checkbox-active-nosubstrat.svg`, parsed via `SVGPathParser`
- 24x24dp indicator, 8dp gap to label, label styled as Body 5-R (14sp Roboto Regular)
...
checkbox.configure(
    text = "Label",
    shape = CheckboxView.Shape.SQUARE,
    isChecked = false,
    isEnabled = true,
    showText = true,
    colorScheme = brand.checkboxColorScheme(isDark)
)
checkbox.onCheckedChange = { checked -> /* handle change */ }
```
...
5. **Add preview screen** in `app/` module
   ...
   - Add routing in `MainActivity.kt` (`when (componentId)` dispatch)
...
- **New components**: follow the 7-step checklist above -- create View class, color scheme, JSON spec, preview screen, routing, catalog entry, metadata
- **Testing**: build with `./gradlew assembleDebug`, install with `./gradlew installDebug`
- **Naming**: PascalCase for components (`ChipsView`), camelCase for properties, kebab-case for icon names
- **JAVA_HOME**: must be set to Android Studio JBR for Gradle builds: `JAVA_HOME="/Applications/Android Studio.app/Contents/jbr/Contents/Home"`
